               ...
               switch ($field_type->name) {
                    case 'Address':
                         ...
                         if($city != '' && strpos($value, $city) == false) {                      
                              $value = $value . '<a target="_blank" class="pull-right btn btn-xs btn-primary btn-circle" href="http://maps.google.com/?q=' . $value . ', ' . $row->city . '" data-toggle="tooltip" data-placement="left" title="Comprobar la ubicación en el mapa"><i class="fa fa-map-marker"></i></a>';
                         } else if ($value != "") {
                    ...
                    case 'Date':
                         ...
                         $value = $dias[date('w', $dt)] . " " . date('d', $dt) . " de " . $meses[date('n', $dt) - 1] . " del " . date('Y', $dt);
                         break;
                    case 'Datetime':
                         $dt = strtotime($value);
                         ...
                         $dias = array("Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado");
                         $meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
                         $value = $dias[date('w', $dt)] . " " . date('d', $dt) . " de " . $meses[date('n', $dt) - 1] . " del " . date('Y', $dt) . " a las " . date("h:i A", $dt);
                         break;
                    case 'Decimal':
